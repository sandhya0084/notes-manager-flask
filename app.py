...
def forgot_password():
    if request.method == 'POST':
        ...
        if not email:
            ...
            try:
                send_email(email, "Reset Password", f"Click: {reset_url}")
...
@app.route('/view_files')
def view_files():
    return render_template('view_files.html', files=get_user_files(session['user_id']))
...
def search():
    notes = []
    if request.method == "POST":
        notes = search_notes(request.form.get('query'), session['user_id'])
